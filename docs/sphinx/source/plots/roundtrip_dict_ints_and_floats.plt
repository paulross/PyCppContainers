set logscale x
set grid
set title "Time to convert a Python dict to a C++ std::unordered\_map then back to a Python dict."
...
set output "images/roundtrip_dict_ints_and_floats_time.svg"   # choose the output device

plot "dat/roundtrip_dict_int_int.dat" using 1:(1e6 * ($4 - $6)):(1e6 * $3):(1e6 * $7):(1e6 * ($4 + $6)) \
...
set terminal svg size 1400,700           # choose the file format
set output "images/roundtrip_dict_ints_and_floats_rate.svg"   # choose the output device

plot "dat/roundtrip_dict_int_int.dat" using 1:(1e6 * ($4 - $6) / $1):(1e6 * $3 / $1):(1e6 * $7 / $1):(1e6 * ($4 + $6) / $1) \
        t "Dict [int, int] <-> C++ <long, long>" with candlesticks whiskerbars 0.5,\

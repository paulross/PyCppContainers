set logscale x
set grid
set title "Time to convert a Python dict [bytes, bytes] to a C++ std::map<std::vector<char>, std::vector<char>> then back to a Python dict."
...
set output "images/roundtrip_dict_map_bytes_bytes_Byte_length_time.svg"   # choose the output device

plot "dat/roundtrip_dict_map_bytes_bytes_Byte_length_16.dat" using 1:(1e6 * ($4 - $6)):(1e6 * $3):(1e6 * $7):(1e6 * ($4 + $6)) \
        t "Dict [bytes, bytes] <-> C++ std::map<std::vector<char>, std::vector<char>> Length 16" with candlesticks whiskerbars 0.5, \
    "dat/roundtrip_dict_map_bytes_bytes_Byte_length_128.dat" using 1:(1e6 * ($4 - $6)):(1e6 * $3):(1e6 * $7):(1e6 * ($4 + $6)) \
        t "Dict [bytes, bytes] <-> C++ std::map<std::vector<char>, std::vector<char>> Length 128" with candlesticks whiskerbars 0.5, \
    "dat/roundtrip_dict_map_bytes_bytes_Byte_length_1024.dat" using 1:(1e6 * ($4 - $6)):(1e6 * $3):(1e6 * $7):(1e6 * ($4 + $6)) \
        t "Dict [bytes, bytes] <-> C++ std::map<std::vector<char>, std::vector<char>> Length 1024" with candlesticks whiskerbars 0.5, \
    "dat/roundtrip_dict_map_bytes_bytes_Byte_length_16.dat" using 1:(rate_100_000($1) * 1e6) t sprintf("Guide: %.3f µs + 0.1m objects/s", latency*1e6) with lines lw 2 dashtype 5, \
    "dat/roundtrip_dict_map_bytes_bytes_Byte_length_16.dat" using 1:(rate_1_000_000($1) * 1e6) t sprintf("Guide: %.3f µs + 1m objects/s", latency*1e6) with lines lw 2 dashtype 5, \
    "dat/roundtrip_dict_map_bytes_bytes_Byte_length_16.dat" using 1:(rate_10_000_000($1) * 1e6) t sprintf("Guide: %.3f µs + 10m objects/s", latency*1e6) with lines lw 2 dashtype 5

set terminal png size 800,600           # choose the file format
set output "images/roundtrip_dict_map_bytes_bytes_Byte_length_time.png"   # choose the output device
...
set terminal svg size 800,600           # choose the file format
set output "images/roundtrip_dict_map_bytes_bytes_Byte_length_rate.svg"   # choose the output device

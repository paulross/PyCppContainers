...
def get_image_files() -> typing.Set[str]:
    """Return the set of images in docs/sphinx/source/plots/images."""
    # Images are in docs/sphinx/source/plots/images
    ret = set()
    for root, dirs, files in os.walk(os.path.join(DIR_PATH, 'plots', 'images')):
        for name in files:
            ret.add(os.path.abspath(os.path.join(root, name)))
    return ret
...
def main() -> int:
    ...
    print(f'      .dat files: {len(dat_files):8d}')
    print(f' .plt .dat files: {len(plt_dat_files):8d}')
    print(f'.plt image files: {len(plt_img_files):8d}')
    print(f'      .rst files: {len(rst_files):8d}')
    image_files = get_image_files()
    print(f'     Image files: {len(image_files):8d}')

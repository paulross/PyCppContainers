...
/**
 * Create a C++ \c std::string of the length \c string_size then test converting this to a Python \c bytes this
 * \c repeat times.
 * The timing for each test is made by summing the conversion \c test_count times.
 * A single \c TestResult is added to the \c tests_results which will have \c repeat individual execution timings.
 *
 * @param test_results The list of test results to add a single TestResult.
 * @param string_size The length of the string to convert.
 * @param test_count The number of times the conversion is made for the measured execution time. Divide the measured
 *  execution time by this number to get the individual conversion execution time.
 *  Chose this to be higher for fast conversions, lower (or even unity) for slow ones.
 * @param repeat The number of individual execution timings that make up a single test.
 *  Chose this to give a decent min/mean/std. dev./max.
 *  About 5 seems about right.
 * @return Zero.
 */
int
...
int test_vector_vector_char_to_py_list_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_list_like_vector_char_to_py_list_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                       "std::vector");
}
...
test_py_list_bytes_to_vector_vector_char_multiple(TestResultS &test_results, size_t size, size_t str_len,
                                                  size_t repeat) {
    return test_py_list_bytes_to_list_like_vector_char_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                             "std::vector");
}
...
test_py_list_bytes_to_list_vector_char_multiple(TestResultS &test_results, size_t size, size_t str_len,
                                                size_t repeat) {
    return test_py_list_bytes_to_list_like_vector_char_multiple<std::list>(test_results, size, str_len, repeat,
                                                                           "std::list");
}
...
int test_vector_u16string_to_py_list_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_list_like_u16string_to_py_list_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                     "std::vector");
}
...
int test_vector_u32string_to_py_list_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_list_like_u32string_to_py_list_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                     "std::vector");
}
...
test_py_list_str_to_vector_string_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_py_list_str_to_list_like_string_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                      "std::vector");
}
...
test_py_list_str16_to_vector_u16string_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_py_list_str16_to_list_like_u16string_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                           "std::vector");
}

int
test_py_list_str16_to_list_u16string_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_py_list_str16_to_list_like_u16string_multiple<std::list>(test_results, size, str_len, repeat,
                                                                         "std::list");
}
...
test_py_list_str32_to_vector_u32string_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_py_list_str32_to_list_like_u32string_multiple<std::vector>(test_results, size, str_len, repeat,
                                                                           "std::vector");
}

int
test_py_list_str32_to_list_u32string_multiple(TestResultS &test_results, size_t size, size_t str_len, size_t repeat) {
    return test_py_list_str32_to_list_like_u32string_multiple<std::list>(test_results, size, str_len, repeat,
                                                                         "std::list");
}
...
int test_cpp_std_unordered_map_to_py_dict_string_multiple(TestResultS &test_results, size_t size, size_t str_len,
                                                          size_t repeat) {
    return test_cpp_std_map_like_to_py_dict_string_multiple<std::unordered_map>(test_results, size, str_len, repeat,
                                                                                "std::unordered_map");
}
...
void test_performance_all(TestResultS &test_results) {
    ...
    {
        size_t fundamental_types_test_count = 1000 * 1000;
        size_t fundamental_types_test_count_array = 100;
        size_t fundamental_types_test_repeat = TEST_REPEAT * 4;
        size_t fundamental_types_test_repeat_array = TEST_REPEAT;
        ...
        // boolean
        {
            ...
            test_bool_to_py_bool_multiple(test_results, fundamental_types_test_count, fundamental_types_test_repeat);
            test_py_bool_to_cpp_bool_multiple(test_results, fundamental_types_test_count,
        ...
        // Integer
        {
            ...
            test_long_to_py_int_multiple(test_results, fundamental_types_test_count, fundamental_types_test_repeat);
            test_py_int_to_cpp_long_multiple(test_results, fundamental_types_test_count, fundamental_types_test_repeat);
            std::cout << rss_inner << std::endl;
        }
#endif
        ...
        // Floating point
        {
            ...
            test_double_to_py_float_multiple(test_results, fundamental_types_test_count, fundamental_types_test_repeat);
            test_py_float_to_cpp_double_multiple(test_results, fundamental_types_test_count,
                                                 fundamental_types_test_repeat);
            std::cout << rss_inner << std::endl;
        }
#endif
        ...
        // Complex numbers
        {
            ...
            test_complex_to_py_complex_multiple(test_results, fundamental_types_test_count,
                                                fundamental_types_test_repeat);
            test_py_complex_to_cpp_complex_multiple(test_results, fundamental_types_test_count,
                                                    fundamental_types_test_repeat);
            std::cout << rss_inner << std::endl;
        }
#endif
        ...
        // Strings of bytes of various sizes.
        {
            ...
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_cpp_vector_char_to_py_bytes_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                          fundamental_types_test_repeat_array);

            }
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_py_bytes_to_cpp_vector_char_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                          fundamental_types_test_repeat_array);
        ...
        // Strings of various sizes.
        {
            ...
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_cpp_string_to_py_str_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                   fundamental_types_test_repeat_array);
            }
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_py_str_to_cpp_string_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                   fundamental_types_test_repeat_array);
            }
            std::cout << rss_inner << std::endl;
        }
        ...
        // Strings of various sizes.
        {
            ...
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_cpp_u16string_to_py_str16_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                        fundamental_types_test_repeat_array);
            }
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_py_str16_to_cpp_u16string_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                        fundamental_types_test_repeat_array);
            }
            std::cout << rss_inner << std::endl;
        }
        ...
        // Strings of various sizes.
        {
            ...
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_cpp_u32string_to_py_str32_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                        fundamental_types_test_repeat_array);
            }
            for (ssize_t string_size = 1; string_size <= 1 << 20; string_size *= 2) {
                test_py_str32_to_cpp_u32string_multiple(test_results, string_size, fundamental_types_test_count_array,
                                                        fundamental_types_test_repeat_array);
            }
            std::cout << rss_inner << std::endl;
        }
    ...
    test_perf_cpp_std_map_to_py_dict_multiple<long, long>(test_results, "<long,long>", TEST_REPEAT);
#endif
#ifdef TEST_PERFORMANCE_OBJECT_DOUBLE
    test_perf_cpp_std_unordered_map_to_py_dict_multiple<double, double>(test_results, "<double,double>", TEST_REPEAT);
    test_perf_cpp_std_map_to_py_dict_multiple<double, double>(test_results, "<double,double>", TEST_REPEAT);
#endif
#ifdef TEST_PERFORMANCE_OBJECT_COMPLEX
